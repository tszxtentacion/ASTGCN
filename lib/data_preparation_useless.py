...
def read_and_generate_dataset(graph_signal_matrix_filename,
    ...
    for idx in range(data_seq.shape[0]):
        ...
        all_samples.append((
            # 扩展纬度是为了方便后面与其他数据concatenate
            # transpose((0, 2, 3, 1)) changes the order of dimensions in the array.
            # The original shape of week_sample is (num_of_weeks * points_per_hour, num_of_vertices, num_of_features).
            # After transposing,the shape becomes (1, num_of_vertices, num_of_features, num_of_weeks * points_per_hour).
            # The purpose of this transpose is to match the desired shape for the input data of a CNN, where the
            # dimensions conventionally represent batch size, height, width, and channels.
    ...
    # 下面的写法非常奇怪，不知道为什么不能直接用返回值赋值
    norms = normalization(train_week, val_week, test_week)
    week_stats, train_week_norm, val_week_norm, test_week_norm = norms[0], norms[1], norms[2], norms[3]

    norms = normalization(train_day, val_day, test_day)
    day_stats, train_day_norm, val_day_norm, test_day_norm = norms[0], norms[1], norms[2], norms[3]

    norms = normalization(train_hour, val_hour, test_hour)
    recent_stats, train_recent_norm, val_recent_norm, test_recent_norm = norms[0], norms[1], norms[2], norms[3]
