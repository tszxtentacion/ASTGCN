...
def compute_val_loss_mstgcn(net, val_loader, criterion, masked_flag, missing_value, sw, epoch, limit=None):
    # global_step: int, current global_step
    """
    Parameters
    ----------
    net
...
def predict_and_save_results_mstgcn(net, data_loader, data_target_tensor, global_step, metric_method, _mean, _std,
                                    params_path, type_):
    """
    ...
    with torch.no_grad():
        ...
        inputs = re_normalization(inputs, _mean, _std)
        ...
        output_filename = os.path.join(params_path, 'output_epoch_%s_%s' % (global_step, type_))
        np.savez(output_filename, input=inputs, prediction=prediction, data_target_tensor=data_target_tensor)
